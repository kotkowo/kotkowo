defmodule KotkowoWeb.LostAndFoundLive.LostLive.Index do
  ...
  def handle_params(params, _uri, socket) do
    ...
    {:ok, %Paged{items: cats, page_count: page_count, page_size: page_size, page: page, total: total}} =
      [page: page, page_size: page_size, filter: filter] |> Client.new() |> Client.list_lost_cats()
    ...
    socket =
      if page <= page_count do
        ...
        push_navigate(socket, to: ~p"/zaginione-znalezione/zaginione")
      end
  ...
  def handle_info({:filter_cat, %Cat.Filter{} = filter}, socket) do
    {:noreply,
     push_patch(socket, to: ~p"/zaginione-znalezione/zaginione" <> "?#{Cat.Filter.to_params(filter)}", replace: true)}
  end
