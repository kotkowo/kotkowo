defmodule KotkowoWeb.NewsLive.PassedAway do
  ...
  def handle_event("items_amount", %{"items_per_page" => amount}, socket) do
    ...
    socket =
      push_patch(socket,
        to: ~p"/aktualnosci/za-teczowym-mostem?#{params}" <> "#{Cat.Filter.to_params(socket.assigns.filter)}",
        replace: true
      )
      ...
      case cats do
        {:ok, %Paged{items: cats, page_count: page_count, page_size: page_size, page: page, total: total}} ->
          ...
          if page <= page_count do
            ...
            push_patch(socket,
              to: ~p"/aktualnosci/za-teczowym-mostem" <> "?#{Cat.Filter.to_params(socket.assigns.filter)}"
            )
          end
  ...
  def handle_info({:filter_cat, %Cat.Filter{} = filter}, socket) do
    {:noreply,
     push_patch(socket,
       to: ~p"/aktualnosci/za-teczowym-mostem?#{socket.assigns.params}" <> "#{Cat.Filter.to_params(filter)}",
       replace: true
     )}
  end
